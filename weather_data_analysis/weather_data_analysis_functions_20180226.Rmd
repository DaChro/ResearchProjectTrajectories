...
* Value:
    + `RasterStack` object with the corresponding cumulated precipitation values. For each temporal unit (e.g. day) within the target time interval as specified by `interval`, one layer will be created. The slot `z` contains the names of the corresponding time unit.
    
```{r totErainterimPrecipitation, eval = F}
totErainterimPrecipitation <- function(precipitation, time = "day", interval = NULL, cores = 10){
  ...
  z <- strftime(as.POSIXct(precipitation@z[[1]], format = "%Y-%m-%d %H:%M:%S"), format = ft)
  ...
  # define z slot
  precipitation.sum@z[[1]] <- z
  
  # stop cluster
  stopCluster(cl)
  
  # return result
  return(precipitation.sum)
}
```
...
* Value:
    + `RasterStack` object with values for the relative air humidity [%] over water. The temporal resolution is th same as for the input raster objects `dt` and `t`.
    
```{r relErainterimAirhumidity, eval = F}
relErainterimAirhumidity <- function(dt, t, interval = NULL, cores = 10){
  ...
  z <- as.POSIXct(dt@z[[1]], format = "%Y-%m-%d %H:%M:%S")
...
* Value:
    + `RasterStack` object with daily mean values for `variable`.
    
```{r dailyMeans, eval = F}
dailyMeans <- function(variable, cores = 10){
  ...
  z <- as.POSIXct(variable@z[[1]], format = "%Y-%m-%d %H:%M:%S")
...
* Value:
    + `RasterStack` object with monthly long-term mean values for `variable`.
    
```{r monthlyLongTermMeans, eval = F}
monthlyLongTermMeans <- function(variable, tstart, tend, cores = 10){
  ...
  # define z slot
  mean.months@z[[1]] <- unique(z.month)
...
tenDayLongTermMWMeans <- function(variable, tstart, tend, cores = 10){
  ...
  # get vector with leap years
  leap.year <- rep(0, length(z.year))
  leap.year[which(leap.year %in% names(table(z.year))[which(as.numeric(table(z.year)) == 364)])] <- 1
  as.numeric(table(z.year))
  ...
  mean.tds <- 
    foreach(step_i = unique(z.day), .packages = c("raster"), .combine = stack) %dopar%{
      
      # get index to subset rasters
      subset.index <- which(z.day == step_i)
      if(step_i == "02-29"){
        subset.index <- c(subset.index, which(z.day == "02-28" & leap.year == 1))
      }
