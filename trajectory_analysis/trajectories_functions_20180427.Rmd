...
locationsTracks <- function(trs,
  ...
  # define number of visits per location
  visits <- rep(NA, nrow(campsites.var))
  for(loc_i in seq_along(names(table(campsites.var$location)))){
  ...
  # set visits of gaps to NA
  campsites.var$visits[which(campsites.var$location == 0)] <- NA
  ...
  if(return_summary == T && remove_gaps == T && merge_inter_gaps == F){
    campsites.var <- campsites.var[-which(is.na(campsites.var1$visits)),]
    campsites.var1 <- campsites.var1[-which(is.na(campsites.var1$visits)),]
  }
