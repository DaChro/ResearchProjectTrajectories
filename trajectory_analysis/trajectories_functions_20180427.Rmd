...
exDataTracks <- function(trs
  ...
  df <- do.call(rbind, lapply(trs@tracks, function(x){
    df <- x@data
...
* Issues: A solution has to be found for the case that only one location is present in a `Tracks` object. (will not result in a `Tracks` object. Additionally, the data gathering is not straightforward in this case).
...
locationsTracks <- function(trs,
  ...
  while(length(leftover) != 0){
    ...
    # adjust indices of intersecting points
    ind.sub1 <- unique(a.new[which(is.na(a.tot) == F & is.na(a.new) == F)])
    ind.sub2 <- unique(a.tot[which(is.na(a.tot) == F & is.na(a.new) == F)])
    for(i in c(1:length(ind.sub1))){
      a.new[which(a.new == ind.sub1[i])] <- ind.sub2[i] 
    }
    
    # merge a.tot and a.new
    a.tot[which((is.na(a.tot) == F & is.na(a.new) == F)|(is.na(a.tot) & is.na(a.new) == F))] <- a.new[which((is.na(a.tot) == F & is.na(a.new) == F)|(is.na(a.tot) & is.na(a.new) == F))]
  ...
  for(fi in c(1:length(a.tot))){
    ...
    if(fi == length(a.tot) && length(order.campsite.vis) == 1){
      order.campsite.vis <- c(order.campsite.vis, fi)
    }
  }
