...
* Value: 
    + `SpatialPolygonsDataFrame` holding the cells determined as clusters
...
Function in order to identify distinct locations in a `Tracks` object and the order of the visit of these locations and in order to compute various variables concerning the visit of a specific location (mean latitude, mean longitude, mean elevation, residence time, arrival time, departure time, speed). The identification of locations is based on the identification of clusters of fixes by idntifying intersecting buffers around each fix. Campsites are distinguished from other locations by a specified minimal residence time.
...
locationsTracks <- function(trs, # a Tracks object
                            radius = 200,
                            t.min = 345600
                      ){
  ...
  trsSP <- SpatialPointsDataFrame(trsSP, trs.df)
  
  # calculate buffer
  buffer.trs <- gBuffer(trsSP, width = radius, quadsegs = 25)
  
  # find intersecting buffers
  a <- over(trsSP, disaggregate(buffer.trs))
  
  # add variable to trsSP
  trsSP$campsites <- a
  
  # find order of campsite visits
  for(fi in c(1:length(a))){
    if(fi == 1){
      order.campsite.vis <- fi
    }else{
      if(a[fi] != a[order.campsite.vis[length(order.campsite.vis)]]){
  ...
  order.campsite.vis <- list(apply(order.campsite.vis, 2, function(x){x[1]:x[2]}))[[1]]
  ...
  campsites.var$lat <- sapply(order.campsite.vis, function(x){mean(trsSP$lat[x])})
  campsites.var$lon <- sapply(order.campsite.vis, function(x){mean(trsSP$lon[x])})
