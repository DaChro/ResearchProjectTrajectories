...
Function to to determine topology for grid and quadratcount, i.e. size in x and y such that the window of the ppp can be split in an even number of quadrats,approximating a certain target size, e.g. 1000m. This function is used by function `pointsperquad` to create the cells in which to count the number of points.
...
Function to determine points per cell (of a given size) in a tracks-object. Takes a `Tracks` object and a target size of the cells as input and returns a `SpatialGridDataFrame` with quadrat cells and the number of points within them. This function is used by function `extractClusters` to create count the number of points within cells before determining clusters of points.
...
Function to extract clusters using the function `pointsPerQuad`. Takes a cellsize (for the cells in which to count the points) and a threshold for the number of points per cell as arguments.
...
```{r estractClusters, eval = T, message = F, warning = F}
extractClusters <-
