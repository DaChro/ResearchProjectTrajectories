...
gapDurationTrack <- function(track, timeinterval = 30 * 60){
  ...
  duration_intervals <- table(track@data$id_timeinterval[which(track@data$gap == FALSE)]) * timeinterval
...
summaryIndicatorsTrack1 <- function(track, normalize = T, timeinterval = 30*60){
  ...
  summaryindicators <- apply(summaryindicators, 1, function(x){
    
    if(track@data$remove[x[1]] == TRUE){
      c(x[1:3],NA, NA, NA, NA, NA)
    }else{
    ...
    summaryindicators1 <- c(x[1:3], distance_tot, altitude_tot, altitude_dist, campsite_time, nocampsite_time)
    
    # normalize values
    if(normalize == TRUE){
      summaryindicators1[4:length(summaryindicators1)] <- summaryindicators1[4:length(summaryindicators1)]/track@data$duration_nogap[x[2]]
    }
...
summaryIndicatorsTracks1 <- function(tracks, normalize = T, timeinterval = 30*60, cores, clcall){
  ...
  clusterCall(cl, function(){library("spacetime")})
  clusterCall(cl, function(){library("trajectories")})
  clusterExport(cl = cl, varlist = list("tracks", "noralize", "timeinterval", "summaryIndicatorsTrack1"), envir=environment())
